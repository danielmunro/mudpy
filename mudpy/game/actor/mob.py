...
class Mob(actor.Actor):
    ...
    def _decrement_movement_timer(self):
        ...
        if self.movement_timer < 0:
            direction = random.choice([direction for direction, _room in
                ...
                room.get(_room).area == self.get_room().area])
            command.do(self, direction)
            self.movement_timer = self.movement
    ...
    def _die(self):
        ...
        self.publisher.on('tick', self._respawn)

    def _respawn(self):
        self.respawn_timer -= 1
        if self.respawn_timer < 0:
            command.do(self, "wake")
            self.publisher.off('tick', self._respawn)
